...
bool ThreadBase::spawn_thread(shared_ptr<ThreadBase> thread)
{
	boost::timed_mutex::scoped_lock l(children_lock);
	return do_spawn_thread(thread);
}

bool ThreadBase::add_child(shared_ptr<ThreadBase> thread)
{
	boost::timed_mutex::scoped_lock l(children_lock);
...
void ThreadBase::finish_thread(pThreadBase child, bool join)
{
	boost::timed_mutex::scoped_lock l(children_lock);
...
void ThreadBase::join_thread(pThreadBase child)
{
	boost::timed_mutex::scoped_lock l(children_lock);
...
/// @param thread instance of ThreadBase to be run in spawned thread
/// @return true on success, false otherwise
bool ThreadBase::do_spawn_thread(shared_ptr<ThreadBase> thread)
{
	return do_add_child(thread,true);
}
bool ThreadBase::do_add_child(shared_ptr<ThreadBase> thread, bool spawned)
{
	assert(thread.get());
	shared_ptr<ThreadChild> child;
	shared_ptr<boost::thread> t;
	if (spawned) {

		try {
			t.reset(new boost::thread(ThreadSpawn(thread)));
		}
...
void ThreadBase::do_request_finish_thread(pThreadBase child)
{
...
weak_ptr<ThreadBase> ThreadBase::get_this_ptr()
{
