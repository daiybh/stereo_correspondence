...
shared_ptr<BasicPipe> BasicPipe::generator(Log &log,std::string name, Parameters &parameters)
{
...
BasicPipe::BasicPipe(Log &log_, std::string name):log(log_),name(name),
...
void BasicPipe::do_set_limit(yuri::size_t l, yuri::ubyte_t policy)
{
	if (l <= 0 || policy == YURI_DROP_NONE) {
		...
		log[info] << "Removing limits from the pipe" << "\n";
	} else {
		limit = l;
		dropPolicy = policy;
...
void BasicPipe::do_notify()
{
	...
	if (!notifySockets.get()) {
		log[error] << "No notification sockets!!!!!" << "\n";
		return;
	}
	char c = 0x37;
