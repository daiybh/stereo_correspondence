...
struct scale_line_bilinear_fast {
	inline static void eval(uint8_t* it, const uint8_t* top, const uint8_t* bottom, const dimension_t new_width, const dimension_t old_width, const uint64_t unscale_x, const uint64_t y_ratio) {
		...
		for (dimension_t pixel = 0; pixel < new_width -1; ++pixel) {
			...
			for (size_t i = 0; i<pixel_size;++i) {
				*it++ = static_cast<uint8_t>(
					(top[left/256 * pixel_size + i] * x_ratio2 * y_ratio2 +
					top[right/256 * pixel_size + i] * x_ratio * y_ratio2 +
					bottom[left/256 * pixel_size + i] * x_ratio2 * y_ratio +
					bottom[right/256 * pixel_size + i] * x_ratio * y_ratio)/65536);
			}
		}
		for (size_t i = 0; i<pixel_size;++i) {
			*it++ = static_cast<uint8_t>(
				(top[(old_width - 1) * pixel_size + i] * y_ratio2 +
				bottom[(old_width - 1) * pixel_size + i] * y_ratio)/256);
		}
	}
};
...
inline uint8_t get_y_fast (const dimension_t pixel, const uint64_t unscale_x, const uint8_t* top, const uint8_t* bottom, const uint64_t y_ratio, const uint64_t y_ratio2)
{
	const dimension_t left = pixel*unscale_x;
...
//	std::cout << "pixel: " << pixel << ", left:"<<left << ", leftx: " << left/256 * 2 <<std::endl;
	return static_cast<uint8_t>(
		(top[left/256 * 2 + 0] * x_ratio2 * y_ratio2 +
		top[right/256 * 2 + 0] * x_ratio * y_ratio2 +
		bottom[left/256 * 2 + 0] * x_ratio2 * y_ratio +
		bottom[right/256 * 2 + 0] * x_ratio * y_ratio)/65536);
}
template<size_t adjust>
inline uint8_t get_uv_fast (const dimension_t pixel, const uint64_t unscale_x, const uint8_t* top, const uint8_t* bottom, const uint64_t y_ratio, const uint64_t y_ratio2)
{
	...
	const uint64_t x_ratio2 = 256 - x_ratio;
	return static_cast<uint8_t>(
		(top[left/256 * 2 + 1] * x_ratio2 * y_ratio2 +
		top[right/256 * 2 + 1] * x_ratio * y_ratio2 +
		bottom[left/256 * 2 + 1] * x_ratio2 * y_ratio +
		bottom[right/256 * 2 + 1] * x_ratio * y_ratio)/65536);
}
...
template<class kernel>
core::pRawVideoFrame scale_image_fast(const core::pRawVideoFrame& frame, const resolution_t new_resolution)
{
	auto outframe = core::RawVideoFrame::create_empty(frame->get_format(), new_resolution);
	const auto res = frame->get_resolution();
	...
	for (dimension_t line = 0; line < new_resolution.height -1 ; ++line) {
		...
		kernel::eval(it,
				it_in + top /256 * linesize_in,
				it_in + bottom/256 * linesize_in,
				new_resolution.width,
				res.width,
				unscale_x,
				y_ratio);
