...
bool BlankGenerator::set_param(Parameter &p)
{
	if (iequals(p.name,"fps")) {
		...
		format = BasicPipe::get_format_from_string(p.get<string>());
		if (format == YURI_FMT_NONE) {
			log[error] << "Failed to parse format string!" << endl;
			return false;
		}
...
void BlankGenerator::run()
{
	...
	shared_ptr<BasicFrame> frame(new BasicFrame(finfo->planes));
...
shared_ptr<BasicFrame> BlankGenerator::generate_frame()
{
	...
	log[error] << "Wrong format" << endl;
	return shared_ptr<BasicFrame>();
}

shared_ptr<BasicFrame> BlankGenerator::generate_frame_yuv422()
{
	assert(format==YURI_FMT_YUV422);
	shared_ptr<BasicFrame> frame = BasicIOThread::allocate_empty_frame(YURI_FMT_YUV422,width,height);
...
shared_ptr<BasicFrame> BlankGenerator::generate_frame_rgb()
{
	assert(format==YURI_FMT_RGB24);
	shared_ptr<BasicFrame> frame = BasicIOThread::allocate_empty_frame(YURI_FMT_RGB,width,height);
	yuri::ubyte_t *data = (*frame)[0].data.get();
	for (yuri::ushort_t y = 0; y<height; ++y) {
		for (yuri::ushort_t x = 0; x < width; ++x) {
			*data++=0;
